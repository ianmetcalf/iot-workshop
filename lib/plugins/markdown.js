    ...
    Object.keys(files).forEach(file => {
      ...
      if (ext === '.md') {
        ...
        if (t1 && t1.type === 'heading' && t1.depth === 1) {
          site.title = t1.text || site.title;
          tokens.shift();

          if (t2 && t2.type === 'paragraph') {
            site.description = t2.text || site.description;
            tokens.shift();
          }

          Object.assign(data, {
            site: Object.assign({}, meta.site, site),
          });
        }

        files[html] = Object.assign(data, {
          contents: new Buffer(marked.parser(tokens, markedOptions)),
        });
      }
    });
